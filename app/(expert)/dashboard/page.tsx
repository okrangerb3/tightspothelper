...
export default async function ExpertDashboard() {
  ...
  if (profile?.role !== 'expert') redirect(`/${profile?.role}/dashboard`)

  const { data: expert }   = await supabase.from('expert_profiles')
    .select('status,hourly_rate,rating_avg,rating_count,session_count,available').eq('id', user.id).single()

  const { data: sessions } = await supabase.from('sessions')
    .select('*, category:category_id(name), customer:customer_id(full_name)')
    .eq('expert_id', user.id).order('created_at', { ascending: false }).limit(20)
